...
async def init_db():
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        await db.execute("""CREATE TABLE IF NOT EXISTS shopping (id INTEGER PRIMARY KEY, text TEXT, done INTEGER, added_by TEXT)""")
...
async def save_to_db():
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        for sid, item in shopping_list.items():
            await db.execute("INSERT INTO shopping (id, text, done, added_by) VALUES (?, ?, ?, ?)",
                             (sid, item["text"], 1 if item["done"] else 0, item.get("added_by", "")))
...
async def load_from_db():
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        shopping_list.clear()
        async with db.execute("SELECT id, text, done, added_by FROM shopping") as cursor:
            async for sid, text, done, added_by in cursor:
                shopping_list[sid] = {"text": text, "done": bool(done), "added_by": added_by}
...
def shopping_menu_kb():
    ...
    b.button(text="✅ Отметить купленной", callback_data="shopping_done_start")
...
def shopping_numbers_kb(action: str):
    b = InlineKeyboardBuilder()
    if action == "done":
        items = [(sid, item) for sid, item in shopping_list.items() if not item["done"]]
    else:
...
def get_start_text():
    ...
    active_shopping = len([i for i in shopping_list.values() if not i["done"]])
    return (
        counters + "\n"
        "💰 <b>Финансы</b>\n"
        ...
        f"   • <b>Всего: {total}</b>\n\n"
        f"🛒 В списке покупок: <b>{active_shopping}</b>\n"
...
def get_shopping_text():
    ...
    for idx, (sid, item) in enumerate(shopping_list.items(), start=1):
        ...
        text += f"{mark} <b>{idx}.</b> {item['text']}\n"
    text += "\n✅ — уже куплено, ▫️ — в списке.\nВыберите действие кнопками ниже."
    return text

def get_shopping_select_text(action: str):
    if action == "done":
        return "✅ <b>Отметить купленной</b>\n\n" + get_shopping_text() + "\n\nВыберите номер покупки:"
...
async def shopping_got_item(message: types.Message, state: FSMContext):
    ...
    if not text:
        ...
        return
    shopping_counter += 1
    shopping_list[shopping_counter] = {"text": text, "done": False, "added_by": message.from_user.first_name}
    await save_to_db()
    await state.clear()
...
@dp.callback_query(F.data == "shopping_done_start")
async def shopping_done_start(callback: types.CallbackQuery):
    await callback.answer()
    active = [(sid, item) for sid, item in shopping_list.items() if not item["done"]]
    if not active:
        await callback.message.edit_text("🛒 Нет активных покупок для отметки.\n\n" + get_shopping_text(),
                                          reply_markup=shopping_menu_kb(), parse_mode="HTML")
        return
    await callback.message.edit_text(
        get_shopping_select_text("done"),
        reply_markup=shopping_numbers_kb("done"),
        parse_mode="HTML")

@dp.callback_query(F.data.startswith("done_shopping_"))
async def shopping_done_chosen(callback: types.CallbackQuery):
    await callback.answer("Куплено! ✅")
    sid = int(callback.data.split("_")[-1])
    if sid in shopping_list:
        shopping_list[sid]["done"] = True
        await save_to_db()
    push_screen(callback.message.chat.id, "shopping")
